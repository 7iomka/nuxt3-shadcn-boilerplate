...
export default function useDependencies(fieldName: Ref<string>) {
  const form = useFormValues();
  // parsed test[0].age => test.age
  const currentFieldName = fieldName.value.replace(/\[\d+\]/g, '');
  ...
  function getSourceValue(dep: Dependency<any>) {
    ...
    const index = getIndexIfArray(fieldName.value) ?? -1;
    ...
    if (index >= 0 && sourceInitial.join(',') === targetInitial.join(',')) {
      const [_currentLast, ...currentInitial] = fieldName.value
        .split('.')
        .toReversed();
